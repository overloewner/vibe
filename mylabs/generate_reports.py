...
def generate_lab4(output_path):
    ...
    _inline_bold_para(doc,
        ...
        "создание и настройка репозитория, подписание коммитов с помощью SSH-ключа, "
        "управление ветками, слияние изменений, работа с удалённым репозиторием "
        "(GitHub/GitLab) и инспекция истории проекта."
    )

    # 2. Теоретическое обоснование
    add_heading(doc, "2. Теоретическое обоснование")
    add_paragraph(doc,
    ...
    concepts = [
        ("Репозиторий (repository)", "хранилище всех версий файлов и метаданных проекта (.git);"),
        ("Коммит (commit)", "снимок состояния проекта в определённый момент времени;"),
        ("Ветка (branch)", "независимая линия разработки, позволяющая вести "
         "параллельную работу без влияния на основную ветку;"),
        ("Слияние (merge)", "объединение изменений из разных веток в одну;"),
        ("Удалённый репозиторий (remote)", "копия репозитория на внешнем сервере "
         "(GitHub, GitLab) для совместной работы."),
    ]
    ...
    add_heading(doc, "3. Используемые инструменты")
    tools = [
        "Python 3.11 — язык написания проекта math_utils;",
        "Git 2.45 — система контроля версий;",
        "GitHub / GitLab — облачные платформы для хостинга репозитория;",
    ...
    # 4. Ход выполнения
    add_heading(doc, "4. Ход выполнения работы")

    # 4a. Структура проекта
    add_heading(doc, "4a. Структура проекта")
    add_paragraph(doc,
        "Проект math_utils содержит модуль математических утилит. "
        "Первоначально реализованы функции: add, subtract, multiply, divide, power, factorial. "
        "В ветке feature/extended-math добавлены функции sqrt и is_prime. "
    ...
    # 4b. Инициализация
    add_heading(doc, "4b. Инициализация репозитория")
    add_paragraph(doc,
        "В рабочей папке проекта выполнена инициализация локального репозитория:"
    )
    add_code_block(doc, "git init\ngit status")
    add_figure_caption(doc, "Рис. 2. Инициализация репозитория (git init) и статус (git status)")

    # 4c. Настройка криптографической подписи
    add_heading(doc, "4c. Настройка криптографической подписи коммитов")
    add_paragraph(doc,
        "Для обеспечения подлинности коммитов сгенерирован SSH-ключ ed25519 "
        "и настроена подпись коммитов:"
    )
    add_code_block(doc,
    ...
    add_figure_caption(doc, "Рис. 3. Настройка SSH-подписи коммитов")

    # 4d. Связь с удалённым репозиторием
    add_heading(doc, "4d. Связь с удалённым репозиторием")
    add_paragraph(doc,
    ...
    # 4e. Первый коммит
    add_heading(doc, "4e. Первый коммит")
    add_paragraph(doc,
        "Добавление файлов в индекс и создание подписанного коммита:"
    )
    add_code_block(doc,
        "git add main.py\n"
    ...
    add_figure_caption(doc, "Рис. 5. Первый подписанный коммит и отправка на GitHub")

    # 4f. Работа с ветками
    add_heading(doc, "4f. Работа с ветками")
    add_paragraph(doc,
        "Для разработки новой функциональности (sqrt, is_prime) создана отдельная ветка "
        "feature/extended-math. В ней выполнено 3 коммита:"
    )
    add_code_block(doc,
        "git checkout -b feature/extended-math\n"
        "# добавлены функции sqrt и is_prime в main.py\n"
        'git commit -S -m "test: добавлены тесты для новых функций"\n'
    ...
    add_figure_caption(doc, "Рис. 6. Работа в ветке feature/extended-math")

    # 4g. Слияние
    add_heading(doc, "4g. Слияние веток (merge)")
    add_paragraph(doc,
        "После завершения разработки в feature/extended-math выполнено слияние с master. "
        "Тип слияния — fast-forward, поскольку в master не было новых коммитов после "
        "создания ветки feature:"
    )
    add_code_block(doc,
        "git checkout master\n"
    ...
    add_figure_caption(doc, "Рис. 7. Слияние ветки feature/extended-math с master")

    # 5. Git log
    add_heading(doc, "5. Визуализация истории коммитов")
    add_paragraph(doc,
        "История коммитов просмотрена командой git log --graph --oneline:"
    )
    add_code_block(doc, "git log --graph --oneline\n\n" + LAB4_GIT_LOG)
    add_figure_caption(doc, "Рис. 8. История коммитов проекта (git log --graph --oneline)")
    add_paragraph(doc,
        "История показывает линейную цепочку коммитов от Initial commit до текущего HEAD. "
        "Ветка feature/extended-math слита в master методом fast-forward без создания "
        "merge-коммита, что сохраняет чистую линейную историю."
    )

    # 6. Вывод
    add_heading(doc, "6. Вывод")
    add_paragraph(doc,
        "В ходе выполнения лабораторной работы приобретены практические навыки "
        "работы с системой контроля версий Git. Реализован полный цикл работы с "
        "репозиторием: инициализация, настройка криптографической подписи (SSH ed25519), "
        "связывание с удалённым репозиторием GitHub, создание коммитов, управление "
        "ветками и слияние."
    )
    add_paragraph(doc,
        "Использование ветки feature/extended-math позволило изолировать разработку "
        "новых функций (sqrt, is_prime) от основной ветки master. Подписание коммитов "
        "SSH-ключом обеспечивает криптографическое подтверждение авторства каждого "
        "изменения в репозитории. Полученные навыки являются фундаментом командной "
        "разработки и управления версиями программных проектов."
    )
